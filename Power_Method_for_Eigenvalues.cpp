...
//  print_results((double*)A, &x[0], &x0[0]);
  Eigenvalue_Power_Method ((double *) A, col, &eigenvalue, &X[0], &B[0],
			   tolerance, Maximum_Number_of_Iterations);
  if (tries >= Maximum_Number_of_Iterations)
    return -1;
  err =
    Eigenvalue_Power_Method ((double *) A, col, &eigenvalue, X, B, tolerance,
			 ...
			 double x0[], double tolerance, int Maximum_Number_of_Iterations)
{
  ...
  *eigenvalue = Inner_Product (px, px0, n);
  ...
  if (tries >= Maximum_Number_of_Iterations)
  {
    ...
    printf("The dominant eigenvalue is %lf.\n", dominant_eigenvalue);
  }
  return tries;
}
