...
int computeLanczosEigenvalues();
int Lanczos(const MatrixXd& A, const VectorXd& v0, int m, MatrixXd& V, MatrixXd& T);
...
int Arnoldi(const MatrixXd& A, const VectorXd& v0, int m, MatrixXd& V, MatrixXd& H) {
    ...
    for (int j = 0; j < m; ++j) {
        w.noalias() = A * V.col(j);
        H.col(j).head(j+1) = V.leftCols(j+1).transpose() * w;  // Single projection
        w.noalias() -= V.leftCols(j+1) * H.col(j).head(j+1);
        
        const double h_norm = w.norm();
        H(j+1, j) = h_norm;
        if (h_norm > 1e-12) V.col(j+1) = w / h_norm;
    }
    return 0;
}

int computeLanczosEigenvalues() {
    cout << "Lanczos Algorithm: " << endl << endl;
    MatrixXd A;
    VectorXd v0;
    if (readMatrixMarketMatrix(A, "A.dat") != 0) {
        cerr << "Failed to read matrix A." << endl;
    ...
    MatrixXd A_sym = (A + A.transpose()) * 0.5;
    printMatrix("Symmetric Matrix", A_sym);
    cout << "Lanczos Tridiagonalization Algorithm:\n\n";
	const int m = std::min(100, static_cast<int>(A.rows()));
    MatrixXd V, T;
    auto start = chrono::system_clock::now();
    int status = Lanczos(A, v0, m, V, T);
    if (status != 0) return status;
    // Diagonalize tridiagonal matrix
    SelfAdjointEigenSolver<MatrixXd> eigensolver(T.topLeftCorner(m, m));
    if (eigensolver.info() != Success) {
        cerr << "Eigen decomposition failed" << endl;
        return -1;
    }
    auto end = chrono::system_clock::now();
    cout << "Tridiagonal Matrix T:\n" << T.topLeftCorner(m, m) << "\n";
...
int Lanczos(const MatrixXd& A, const VectorXd& v0, int m, MatrixXd& V, MatrixXd& T) {
    ...
    if (!A.isApprox(A.transpose(), 1e-8)) {
        cerr << "Matrix must be symmetric for Lanczos\n";
        return -1;
    }
    V.resize(n, m + 1);
    T.resize(m + 1, m);
    T.setZero();
    VectorXd v_current = v0.normalized();
    V.col(0) = v_current;
    VectorXd v_prev; // Stores previous basis vector
    double beta_prev = 0.0;
    for (int j = 0; j < m; ++j) {
        // Matrix-vector multiplication (most expensive operation)
        VectorXd w = A * v_current;
        // Compute diagonal element
        const double alpha = v_current.dot(w);
        T(j, j) = alpha;
        ...
        // Handle Lanczos breakdown
        if (beta < 1e-12) {
            cerr << "Lanczos breakdown at step " << j + 1 << endl;
            return (j == m - 1) ? 0 : -1;
        }
        // Update basis vectors if not last iteration
        if (j < m - 1) {
            const VectorXd v_next = w / beta;
            V.col(j + 1) = v_next;
            v_prev.swap(v_current);  // Swap pointers instead of copying
            v_current = v_next;
            beta_prev = beta;
        }
    }
