...
void Multiply_Matrix_by_Vector (double u[], double *A, int nrows, int ncols,
				double v[]);
int Input_Data (void);
int Parameter_Solve (double array_A[], double B[], double X[], int row,
		      int col, double tolerance, int Maximum_Number_of_Iterations);
int print_results (double array[], double x0[], double x[], int row, int col, double tolerance, int Maximum_Number_of_Iterations);
int
main ()
{
  Input_Data ();
...
//    Change program to use the single index array_A
  ...
  fclose (myFile2);
  Parameter_Solve (&array_A[0], &B[0], &X_k[0], row, col, tolerance, Maximum_Number_of_Iterations);
  delete [] array_A;
  delete [] B;
  delete [] X_k;
  return 0;
...
int Parameter_Solve (double array_A[], double B[], double X[], int row, int col, double tolerance, int Maximum_Number_of_Iterations)
{
  fprintf (stdout, "\n");
  ...
  Eigenvalue_Power_Method (&array_A[0], col, &eigenvalue, &X[0], &B[0], tolerance, Maximum_Number_of_Iterations);
  if (tries >= Maximum_Number_of_Iterations)
    return -1;
  err = Eigenvalue_Power_Method (&array_A[0], col, &eigenvalue, &X[0], &B[0], tolerance, Maximum_Number_of_Iterations);
  if (err == -1)
  ...
  print_results (&array_A[0], &B[0], &X[0], row, col, tolerance, Maximum_Number_of_Iterations);
  return 0;
}


int Eigenvalue_Power_Method (double array_A[], int n, double *eigenvalue, double x[],
			 double x0[], double tolerance, int Maximum_Number_of_Iterations)
{
  int tries;
  double old_estimate;
  double norm;
  ...
  *eigenvalue = Inner_Product (px, px0, n);
  ...
  for (tries = 1; tries < Maximum_Number_of_Iterations; tries++)
    {
