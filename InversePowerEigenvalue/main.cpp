...
void swap_rows(double *A, int num_cols, int row1, int row2);
...
int Equilibrate_Matrix(double *A, double array_A[], int num_rows, int num_cols, double r[], double c[]);
...
//bool ReadVectorMarketFile(vector<double>& B, int& num_cols);
int Write1DArrayToMatrixMarketFile(const double X[], int num_rows);
int Output_Inverse_Power_Results (double array_A[], int num_cols, double X0[],double tolerance, int max_tries);

//void swap_rows(double *A, int num_cols, int row1, int row2) {
  //for (int i = 0; i < num_cols; i++) {
    //double temp = A[i][j]; // Calculate offset for row1
    //A[row1][j] = A[row2][j]; // Calculate offset for row2
    //A[row2][j] = temp;
  //}
//}

void swap_rows(double *A, int num_cols, int row1, int row2) {
  // Loop through each column and swap elements between the rows
  for (int j = 0; j < num_cols; j++) {
    double temp = A[row1 * num_cols + j]; // Calculate offset for row1
    A[row1 * num_cols + j] = A[row2 * num_cols + j]; // Swap element in row1
    A[row2 * num_cols + j] = temp; // Swap element in row2
  }
}
...
//            update remaining matrix
  ...
  //for (int k = 0; k < num_cols; k++) {
    //// Find the pivot row (index) with the largest absolute value in the current column (k)
    //int pivot_row = k;
    //double A_row = A[k]; // Get the kth row as a pointer
    //double max = fabs(A_row); // Access the kth element in the kth row
    //for (int j = k + 1; j < num_cols; j++) {
      //A_row = A[j]; // Get the jth row as a pointer
      //if (fabs(A_row) > max) { // Access element using pointer arithmetic
        //max = fabs(A_row);
        //pivot_row = j;
      //}
    //}
    //// Check for singular matrix (pivot value is zero)
    //if (max == 0.0) return -1;
    //if (pivot_row != k) {
	    //if (0 <= row1 && row1 < num_cols && 0 <= row2 && row2 < num_cols) {
          //swap_rows(A, num_cols, row1, row2);       // Pass the entire array and row indices
        //}     
        //else {
        //// Handle invalid row indices (e.g., print an error message)
        //}
    //}
    //for (int j = k + 1; j < num_cols; j++) {
      //// Update remaining elements in the current column (k)
      //A[j * num_cols + k] /= A[k * num_cols + k];  // Normalize by diagonal element (k, k)
      //A[i * num_cols + j] -= A[i * num_cols + k]  *A[k * num_cols + j]; // Eliminate off-diagonal elements
    //}
    //// Update the remaining sub-matrix (excluding the diagonal and processed column)
    //for (int i = k + 1; i < num_cols; i++) {
      //for (int j = k + 1; j < num_cols; j++) {
        //A[i * num_cols + j] -= A[i * num_cols + k] * A[k * num_cols + j]; // Eliminate off-diagonal elements
      //}
    //} 
...
int Eigenvalue_Inverse_Power_Method(double array_A[], int num_cols, double eigenvalue, double X0[], double tolerance, int max_tries)
{
   ...
   double *x;
   int* pivot;
   pivot = (int*)calloc(static_cast<size_t>(num_cols), sizeof(int));
   ...
   pivot = (int*)malloc(sizeof(int) * static_cast<size_t>(num_cols));
   ...
   Equilibrate_Matrix(*A, &array_A[0], num_cols, num_cols, pr, pc); 
...
std::vector<double> CreateZeroVector(int num_rows) {
    return std::vector<double>(static_cast<std::vector<double>::size_type>(num_rows), 0.0);
}

int Inverse_Power_Input() {
    int num_rows, num_cols, number_of_entries;
    // Read matrix dimensions from A.dat
    std::ifstream file("A.dat");
    if (!file.is_open()) {
        std::cerr << "Error opening file A.dat" << std::endl;
        return -1;
    }
    // Skip header comments (assuming single line)
    std::string line;
    std::getline(file, line);
    // Read dimensions in one step for efficiency
    if (!(file >> num_rows >> num_cols >> number_of_entries)) {
        std::cerr << "Error reading matrix dimensions from A.dat" << std::endl;
        return -1;
    }
    // Create sparse matrix representation (consider using a sparse library for larger matrices)
    std::vector<double> array_A(static_cast<size_t>(number_of_entries));
    std::vector<double> B;
    ReadVectorMarketFile(B, num_cols);  // Assuming this reads B from a file
    // Read non-zero elements directly into sparse A
    int i_index, j_index;
    double elem;
    for (int i = 0; i < number_of_entries; ++i) {
        if (!(file >> i_index >> j_index >> elem)) {
            std::cerr << "Error reading matrix entries from A.dat" << std::endl;
            return -1;
        }
        i_index--;  // Adjust for zero-based indexing
        j_index--;
        array_A[static_cast<size_t>(i)] = elem; // Assuming row-major ordering for sparse A
    }
    // Call Inverse_Power_Solve
    int max_tries = 1000;
...
int Subtract_Scalar_from_Diagonal(double array_A[], double x_scalar, int num_rows, int num_cols)
{
  ...
  return static_cast<int>(array_A[0]);
}
////////////////////////////////////////////////////////////////////////////////
// File: copy_vector.c                                                        //
// Routine(s):                                                                //
...
void Copy_Vector(double *d, double *s, int num_cols)
{
  memcpy(d, s, sizeof(double) * static_cast<size_t>(num_cols));
}
////////////////////////////////////////////////////////////////////////////////
...
int ReadMatrixMarketFile(matrix<double>& A, int& num_rows, int& num_cols) {
  ...
  int i_index, j_index, number_of_entries_A;
  double elem {0.0};
  ...
  // Resize A to accommodate num_rows and num_cols (done once)
  A.resize(static_cast<size_t>(num_rows));
  for (int i = 0; i < num_rows; ++i) {
    A[i].resize(static_cast<size_t>(num_cols));
  }
...
int ReadVectorMarketFile(vector<double>& B, int& num_cols) {
   ...
   for (int i = 0; i < 3; i++)
   {
     fscanf (file, "%d,", &dim_B_Array[i]);
   }
   // Resize the vector to accommodate the number of elements
   num_cols = dim_B_Array[1]; // Assuming num_cols refers to number of columns
   B.resize(static_cast<size_t>(num_cols)); // Pre-allocate space for all elements
   for (int i = 0; i < dim_B_Array[1]; i++)
   {
     fscanf (file, "%d,", &i_index);
     fscanf (file, "%d,", &j_index);
     fscanf (file, "%lf,", &value);
     // Adjust indices for 0-based indexing (assuming desired)
     i_index--;
     j_index--;
     if (value != 0.0) {
		 B[i_index * static_cast<size_t>(num_cols) + j_index] = value; // Calculate index based on row & col
     }
   }
   fclose (file);
 return 0;
...
int Output_Inverse_Power_Results(double array_A[], int num_cols, double X0[], double tolerance, int max_tries) {
    ...
    // Print parameters with formatting
    std::cout << std::setprecision(5) << std::endl;
    std::cout << "Tolerance: " << tolerance << std::endl;
    std::cout << "Max tries: " << max_tries << std::endl;
    // Print header for matrix A
    std::cout << "******************** Solve Ax = B ********************\n"
              << "where the matrix A = \n";
    // Print matrix A with consistent formatting
    for (int i = 0; i < num_rows; ++i) {
        for (int j = 0; j < num_cols; ++j) {
            std::cout << std::setw(6) << std::setprecision(4) << array_A[i * num_rows + j] << "  ";
        }
        std::cout << std::endl;
    }
    // Print normalized eigenvector X0
    std::cout << "\nThe normalized eigenvector is " << std::endl;
    Write1DArrayToMatrixMarketFile(X0, num_rows);
    return 0;
}
