...
int Parameter_Solution(double array_A[], int ncols, double eigenvalue, double X0[],double tolerance, int max_tries);
...
//   double Bx;
   for (i = 0; i < ncols; i++) {
      double Bx = frexp(B[i], &B_exponent);
      double rx = frexp(r[i], &r_exponent);
...
{
  int i, max_tries = 100;
  double eigenvalue = 0.0, tolerance = 0.0001;
  FILE *myFile;
  ...
  fclose (myFile2);
  Parameter_Solution (&array_A[0], ncols, eigenvalue, &X0[0], tolerance, max_tries);
  delete [] array_A;
  delete [] X;
  delete [] X0;
  return 0;
