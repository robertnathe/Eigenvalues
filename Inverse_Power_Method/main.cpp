...
int Output_Inverse_Power_Results (double array_A[], int ncols, double X0[],double tolerance, int max_tries);
int Inverse_Power_Solve(double array_A[], int ncols, double eigenvalue, double X0[],double tolerance, int max_tries);
int Subtract_Scalar_from_Diagonal(double array_A[], double x_scalar, int nrows, int ncols);
void Copy_Vector(double *d, double *s, int ncols);
int Crout_LU_Decomposition_with_Pivoting(double *A, int pivot[], int ncols);
int Crout_LU_with_Pivoting_Solve(double *LU, double B[], int pivot[], double x[], int ncols);
int Equilibrate_Matrix(double *A, int nrows, int ncols, double r[], double c[]);
int Equilibrate_Right_Hand_Side(double B[], double r[], int ncols);
int Unequilibrate_Solution(double x[], double c[], int ncols);
int Multiply_Matrix_with_Vector(double X[], double array_A[], int nrows, int ncols, double X0[]);
int Eigenvalue_Inverse_Power_Method(double array_A[], int ncols, double eigenvalue, double X0[], double tolerance, int max_tries);
...
// Begin your code here.
  err = Eigenvalue_Inverse_Power_Method(&array_A[0], ncols, eigenvalue, &X0[0], tolerance, max_tries);
...
// End your code here.
  Output_Inverse_Power_Results (&array_A[0], ncols, &X0[0], tolerance, max_tries);
  return 0;
}
int Subtract_Scalar_from_Diagonal(double array_A[], double x_scalar, int nrows, int ncols)
{
  ...
  for (i=0;i<n;i++)
  {
    array_A[i*ncols+i] = array_A[i*ncols+i] - x_scalar;
  }
  return array_A[0];
}
////////////////////////////////////////////////////////////////////////////////
...
//         For each row and column, k = 0, ..., n-1,
   for (k = 0, p_k = A; k < ncols; p_k += ncols, k++) {
//            find the pivot row
      pivot[k] = k;
      max = fabs( *(p_k + k) );
      for (j = k + 1, p_row = p_k + ncols; j < ncols; j++, p_row += ncols) {
         if ( max < fabs(*(p_row + k)) ) {
            max = fabs(*(p_row + k));
            pivot[k] = j;
            p_col = p_row;
...
//            update remaining matrix
      for (i = k+1, p_row = p_k + ncols; i < ncols; p_row += ncols, i++)
         for (j = k+1; j < ncols; j++)
...
//         triangular matrix.
   for (k = 0, p_k = LU; k < ncols; p_k += ncols, k++) {
...
//         assumed to be 1.0.
   for (k = ncols-1, p_k = LU + ncols*(ncols-1); k >= 0; k--, p_k -= ncols) {
      if (pivot[k] != k) {dum = B[k]; B[k] = B[pivot[k]]; B[pivot[k]] = dum; }
      for (i = k + 1; i < ncols; i++) x[k] -= x[i] * *(p_k + i);
      if (*(p_k + k) == 0.0) return -1;
   }
   return 0;
}
...
int Eigenvalue_Inverse_Power_Method(double array_A[], int ncols, double eigenvalue, double X0[], double tolerance, int max_tries)
{
   ...
   double *x;
   int* pivot;
   double A[ncols][ncols];
   for (i = 0; i < ncols; i++)
   {
     for (j = 0; j < ncols; j++)
	 {
	   A[i][j] = array_A[i * ncols + j];
	 }
   }
   ...
   norm = Vector_L2_Norm(X0, ncols);
   if ( norm == 0.0 )  return -2;
               // Dynamically allocate working storage memory.
   pivot = (int*) malloc( sizeof(int) * ncols);
   x   = (double*) malloc( sizeof(double) * ncols);
   px0 = (double*) malloc( sizeof(double) * ncols);
   pc  = (double*) malloc( sizeof(double) * ncols);
   pr  = (double*) malloc( sizeof(double) * ncols);
   if (pr == NULL) { error = -4; goto ret; }
                       // Normalize the initial vector.
   Divide_Vector_by_Scalar(X0, norm, ncols);
   ...
   Subtract_Scalar_from_Diagonal(&array_A[0], eigenvalue, ncols, ncols);
   Equilibrate_Matrix(*A, ncols, ncols, pr, pc);
   if ( Crout_LU_Decomposition_with_Pivoting(*A, pivot, ncols) < 0 ) goto ret;
           // Calculate initial estimate of the dominant eigenvalue.
   Copy_Vector(px0, X0, ncols);
   Equilibrate_Right_Hand_Side(px0, pr, ncols);
   Crout_LU_with_Pivoting_Solve(*A, px0, pivot, x, ncols);
   Unequilibrate_Solution(x, pc, ncols);
   lambda = Inner_Product(x, X0, ncols);
...
//    vector<vector<double> > A;
    ...
    Inverse_Power_Solve (&array_A[0], ncols, eigenvalue, &B[0], tolerance, max_tries); 
    array_A.clear();
