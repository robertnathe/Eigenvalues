...
int Output_Inverse_Power_Results(double array_A[], int num_cols, double X0[], double tolerance, int max_tries) {
    ...
    std::cout << "\nThe normalized eigenvector is X0 = " << std::endl;
    PrintVector(&X0[0], num_cols);  // Assuming PrintVector is defined elsewhere

    // Write results to X.dat using a single output stream
    std::ofstream outputFile("X.dat");
    if (!outputFile.is_open()) {
        std::cerr << "Error opening file X.dat" << std::endl;
        return -1;
    }

    outputFile << "%%MatrixMarket_Output_vector_X.dat matrix coordinate pattern general\n";
    outputFile << 1 << " " << num_rows << " " << num_rows << std::endl;
    for (int i = 0; i < num_cols; ++i) {
        outputFile << 1 << " " << i + 1 << " " << X0[i] << std::endl;
    }
...
int Inverse_Power_Input() {
    ...
    std::vector<double> array_A(number_of_entries);
    std::vector<double> B;
    ...
    for (int i = 0; i < number_of_entries; ++i) {
        ...
        i_index--;  // Adjust for zero-based indexing
        j_index--;
        array_A[i] = elem;  // Assuming row-major ordering for sparse A
    }
    ...
    // No need to clear vectors as ownership is transferred to calling function
    return 0;
}
