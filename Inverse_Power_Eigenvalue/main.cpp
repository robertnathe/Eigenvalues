...
int MultiplyMatrixByVector(double X[], double array_A[], int num_rows, int num_cols, double X0[]);
...
int Eigenvalue_Inverse_Power_Method(double array_A[], int num_cols, double eigenvalue, double X0[], double tolerance, int max_tries)
{
   ...
   lambda = CalculateInnerProduct(x, X0, num_cols);
...
int Inverse_Power_Input() {
    ...
    std::vector<double> B;
    ReadMatrixMarketVector(B, num_cols);  // Assuming this reads B from a file
...
double CalculateInnerProduct(double u[], double v[], int n)
{
   ...
   for (n--; n >= 0; n--) CalculateInnerProduct +=  u[n] * v[n];
